...
import { TOPIC_X_URL, SERVER_ERROR, responseBody } from "@callbacks/constants";
...
export const TopicXLandingRequests = {
    getAll: () => 
        instance.get<QuestionDetails[]>("/landing")
        .then(responseBody)
        .catch((error) => {
    ...
    getLimited: (limit: number) =>
        instance.get<QuestionDetails[]>(`/landing?limit=${limit}`)
        .then(responseBody)
        .catch((error) => {
            console.log("error in fetching questions for topic X",error);
            return [] as QuestionDetails[];
